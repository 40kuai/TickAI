...
def _run(
    ...
    if not probe["success"]:
        ...
        # process_restart: systemctl is-active 对非 active 单元返回 exit≠0(3), 这是"服务异常"
        # 信号而非探测失败 → 继续自愈(重启)。实测 CentOS7(systemd 219) 对停止服务输出
        # "unknown"(而非 "inactive"), 故 inactive/failed/unknown/activating/deactivating 均视为
        # 服务不在运行; 单元真不存在时由 systemctl restart 报 "Unit not found" 落 failed 更权威。
        if scene == "process_restart" and probe_out in (
                "inactive", "failed", "unknown", "activating", "deactivating"):
            pass
        else:
